...
describe('UserAnalyticsController', () => {
  ...
  describe('getUserGrowthStatsNonCumulative', () => {
    it('should return 200 with non-cumulative growth stats for admin users', async () => {
      // Mock authenticated request with admin role
      mockReq = {
        user: { role: 'admin' },
        query: {
      ...
      (UserAnalyticsService.prototype.getUserGrowthStatsNonCumulative as jest.Mock)
        .mockResolvedValueOnce(mockGrowthStats);

      // Execute
      await controller.getUserGrowthStatsNonCumulative(
        mockReq as Request,
        mockRes as Response,
        mockNext
      );
